...
export async function executeTrade(
  ...
  try {
    ...
    // halts trading (shared helper). Risk knobs (leverage/size) remain
    // fail-open with safe defaults so a partial config outage does not
    // invent caps.
    const killResult = await checkKillSwitch(env.CONFIG_KV, {
      onMissingKv: "closed",
      onReadError: "closed",
    });
    if (killResult.enabled) {
      if (killResult.source === "missing_kv") {
        throw new Error(
          `${KILL_SWITCH_ACTIVE_PREFIX}: CONFIG_KV not configured — trading halted (fail-closed)`
        );
      }
      if (killResult.source === "read_error") {
        throw new Error(
          `${KILL_SWITCH_ACTIVE_PREFIX}: Unable to verify kill switch (${killResult.error ?? "unknown"})`
        );
      }
      throw new Error(
        `${KILL_SWITCH_ACTIVE_PREFIX}: Trading is disabled by kill switch (source=${killResult.source ?? "unknown"})`
      );
    }

    // Risk knobs: fail-open defaults if CONFIG_KV is present but risk keys fail.
    if (env.CONFIG_KV) {
      try {
        const [defaultLevStr, maxSizeStr] = await Promise.all([
        ...
        logger.error(
          "Failed to fetch trade risk settings from KV — using defaults (fail-open)",
          { error: toError(e) }
        );
      }
    }
